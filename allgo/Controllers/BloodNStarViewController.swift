//
//  BloodNStarViewController.swift
//  allgo
//
...
class BloodNStarViewController: UIViewController {
    ...
    @objc func closeImgClicked(){
        ...
        if from == "NEW"{
            ...
            sajuInfoSavePop.from = Constants.THEME_UNSE_NAME[8]
    ...
    func getBloodNStar(param: Dictionary<String,String>){
        
        let url = Constants.urlPrefix + "/theme/getBloodNStar"
            ...
            .responseJSON { (response) in
                ...
                case .success(let value):
                    ...
                    if errorNumber == Constants.SEARCH_OK{
                        ...
                        let bloodNStar = json["bloodNstar"].arrayValue[0]
//
                        self.conId = bloodNStar["id"].stringValue
                        let keyword = bloodNStar["keyword"].stringValue
                        let starName = bloodNStar["starname"].stringValue
                        let blood = bloodNStar["blood"].stringValue
                        let contents1 = bloodNStar["contents1"].stringValue
                        let contents = bloodNStar["contents"].stringValue
//
                        self.keywordContentLbl.text = keyword
                        self.contentLbl.text = blood + "\t" + starName + "\t" + contents1 + "\n" + contents
                    }
                default:
                    return
